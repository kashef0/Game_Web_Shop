...
import { Box, Button, Flex, Heading, HStack, Image, Separator, Text, VStack } from '@chakra-ui/react';  
...
const Cart = () => {
  ...
  const handleRemove = (id: string) => {  // för att ta bort en vara från varukorgen
    dispatch(removeFromCart(id));
  };

  const handleCheckout = () => {  // för att navigera till checkout sidan
    navigate('/checkout');
  };
          ...
          {items.map((item) => ( 
            ...
            >
              <HStack align="flex-start">
                <Image src={item.game.background_image} boxSize='10rem' fit='fill' rounded='md' /> 
                <Box marginLeft={2}>
                  ...
                  <Text>
                    Price per unit: ${item.isRental ? item.game.rentalPrice : item.game.price} 
                  </Text>
                  {item.isRental && ( 
